const original = `be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
// const original = `badc bd dbeaf cfdbge dfb cfbdea efbag edcfgab dcafe degfca | eacfd acdfbe cbdegf fcbaedg
...
for(let i = 0; i < lines.length; i ++){
  let matters = lines[i].split(' | ')[1].split(' ')
  for(let j = 0; j < matters.length; j++){
    if(!!~[2, 3, 4, 7].indexOf(matters[j].length)){
      total += 1
    }
  }
}
...
//
// // let numbers = ['cagedb', 'ab', 'gcdfa', 'fbcad', 'eafb', 'cdfbe', 'cdfgeb', 'dab', 'acedgfb', 'cefabd']
// // numbers = numbers.map(num => [...num].sort().join(''))
// // console.log(numbers)
// //
...
// //   console.log('******')
// //   for(let j = 0; j < matters.length; j++){
// //     console.log([...matters[j]].sort().join(''))
// //     numberString += numbers.indexOf([...matters[j]].sort().join(''))
// //   }
// //   console.log(numberString)
// //   totalNumbers.push(Number(numberString))
// //
// // }
// // console.log(totalNumbers.reduce((previousValue, currentValue) => previousValue + currentValue, 0))
//
//
// const getAlternativeNumbers = input => {
//   const test = original.replace(/\n/g, '').split('|').map(x => x.trim())
//   const patterns = new Map()
//   for(let i = 0; i < test.length; i++){
//     const line = test[i].split(' ')
//     for(let j = 0; j < line.length; j++){
//       const str = [...line[j]].sort().join('')
//       if(str.length === 5) {
//         if (!patterns.size || !patterns.has(str)) {
//           patterns.set(str, 1)
//         } else {
//           let existing = patterns.get(str)
//           patterns.set(str, existing += 1)
//         }
//       }
//     }
//   }
...
// }
//
// const patterns5 = getAlternativeNumbers(original)
// console.log(patterns5)
// console.log('cdfeb', patterns5.get([...'cdfeb'].sort().join()))
// console.log('fcadb', patterns5.get([...'fcadb'].sort().join()))
// console.log('cdfeb', patterns5.get([...'cdfeb'].sort().join()))
// console.log('cdbaf', patterns5.get([...'cdbaf'].sort().join()))
//
//
// // const test = original.replace(/\n/g, '').split('|').map(x => x.trim())
// // const patterns = new Map()
...
// //   for(let j = 0; j < line.length; j++){
// //     console.log(line[j])
// //     if(!patterns.size || !patterns.has(line[j])){
// //       patterns.set(line[j], 1)
// //     }else{
// //       let existing = patterns.get(line[j])
// //       patterns.set(line[j], existing += 1)
// //     }
// //   }
// // }
// // console.log(patterns)
// // // console.log("unique", unique, "duplicate", duplicate, "difference", Math.abs(unique - duplicate))
//
//
//
//
